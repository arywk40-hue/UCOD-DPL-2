...
Current self-supervised segmentation pipelines suffer from a fundamental structural limitation: they apply global, scalar mixing factors across the entire spatial grid, treating clean object interiors and noisy boundary pixels identically. 
...
\subsection{Latest Validation Result}

\begin{table*}[t]
\centering
\caption{Latest validation result for \methodname{} on TE-CAMO.}
\label{tab:latest_validation}
\resizebox{\textwidth}{!}{%
\begin{tabular}{l|ccccccccc}
\toprule
Method & ACC$\uparrow$ & mIoU$\uparrow$ & $E_\phi^{max}\uparrow$ & $E_\phi^{mean}\uparrow$ & $F_\beta^{max}\uparrow$ & $F_\beta^{mean}\uparrow$ & $S_m\uparrow$ & $M\downarrow$ & $F_\beta^w\uparrow$ \\
\midrule
\textbf{\methodname{} (Ours)} & .8188 & .0000 & .2500 & .2500 & .2182 & .0009 & .4099 & .1812 & .0047 \\
\bottomrule
\end{tabular}%
}
\end{table*}
...
\begin{table*}[t]
\centering
\caption{Comparison with state-of-the-art methods across four COD benchmarks. Quantitative metric values for \methodname{} will be populated post-training.}
\label{tab:sota}
\resizebox{\textwidth}{!}{%
...
in \emph{Proc. Eur. Conf. Comput. Vis. (ECCV)}, 2024.
